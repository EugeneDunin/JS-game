...
function Card(number, value) {
    this.number = number; //Все разные
    this.value = value; //Попарно одинаковые
    this.opened = false; //Открыта на доске
    this.matched = false; //Совпадение найдено
}
...
function OnCardClick(e) {
    if (canBeClicked)
	{
        let elements = Array.from(document.getElementsByClassName("card")); //Выбираем все карты
        for (let i = 0; i < elements.length; i++) //Проходим по всем картам
		{
            if ((e.currentTarget === elements[i]) && (!cardsArray[i].matched)) //Та ли карта и не яв-ся совпадающей
			{
                OpenCard(elements[i], cardsArray[i], `${skinPath}/${cardsArray[i].value}`);
                if (lastOpenedCard === null) //Если нет запомненной последней нажатой карты
				{
